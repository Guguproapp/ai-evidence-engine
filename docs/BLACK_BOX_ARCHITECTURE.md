...
- Development Demo boundary: the public `aee-continuity-demo` service keeps the fixed bundled synthetic continuity path and adds an explicit opt-in Legacy Content Bridge for validated PNG/JPEG/WebP test images under 10 MB. The backend—not the browser—creates the Signed Event and derives all storage identifiers. The service has rate limiting and a maximum of one Cloud Run instance, and uses `aee-continuity-demo@ai-evidence-engine-gugupro.iam.gserviceaccount.com`. That identity has only service-level `roles/run.invoker` on `aee-blackbox-test`; it has no Project or Test-Bucket role. Browser clients never receive a Google credential or identity token. First-Seen local V1/V2 state is ephemeral and is not a Production durability design.
