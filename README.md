...
**Legacy Content Bridge (Development / Test MVP):** an unfamiliar image still returns `UNVERIFIED`, but its owner may explicitly start a verifiable history from the current time. AEE creates a signed First-Seen Event, seals and retrieves the exact bytes through the Development / Test Evidence Black Box, and can link later recorded versions. First-Seen is not proof of originality, authorship, copyright, truth, or any history before AEE received the file. See [Legacy Content Bridge](docs/LEGACY_CONTENT_BRIDGE.md).
